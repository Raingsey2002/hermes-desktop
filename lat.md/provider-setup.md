...
## No more in-app "Active Model" setting

The Providers tab dropped its **ACTIVE MODEL** card (summary + **Change** picker modal + **Browse Registry** button) — it looked contradictory next to the chat-input picker's [[model-selection|real-time session override]].

[[src/renderer/src/screens/Providers/Providers.tsx]] still loads and debounced-auto-saves `modelProvider`/`modelName`/`modelBaseUrl` via `getModelConfig`/`setModelConfig` — that plumbing is kept, unused by any button, purely so an existing `config.yaml` `model:` value (a fresh conversation's and non-chat surfaces' fallback) keeps loading and round-tripping correctly. There is no in-app way to set that default from scratch anymore; users who need one write it via the `hermes` CLI or edit `config.yaml` directly. Models are still added to the library from **within each provider's card** (see "Models live under each provider" below) — that flow is unaffected.

The removed card's supporting code — [[src/renderer/src/screens/Providers/provider-picker.ts#buildAuthenticatedOAuthPickerProviders]], the OAuth/Remote/SSH credential-status plumbing below, and the `LogoSelect` control — is no longer wired to any UI but was left in place (including its dedicated [[tests/provider-picker.test.ts]] and [[tests/remote-provider-statuses.test.ts]] coverage) rather than deleted, in case the picker returns in another form.

### Authenticated OAuth providers were selectable

OAuth plans with usable `auth.json` credentials used to appear in the removed active-model picker with no API-key env var needed; the mechanism still exists and is test-covered, just unreachable from any button now.

Each picker open asked the main process for a boolean status per supported OAuth provider. Local mode uses [[src/main/config.ts#hasOAuthCredentials]] so both `providers` and `credential_pool` auth shapes and profile fallback work. Access and refresh tokens never enter renderer state. [[src/renderer/src/screens/Providers/provider-picker.ts#buildAuthenticatedOAuthPickerProviders]] merges authenticated plans with saved and discovered models, while deduplicating providers such as Nous that may also have an API key. [[tests/provider-picker.test.ts]] covers the keyless Codex-plan regression.
