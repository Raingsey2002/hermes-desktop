...
- [[skills]] — the Skills screen and chat-input picker; gateway-routed list/content in every connection mode, and installing a skill from a URL/identifier or a local file, not just the bundled catalog.
- [[confirmation-ui]] — the structured approval card for mid-turn `approval.request`; the pending-approval registry and run-scoped delivery guard that replaced the dashboard transport's unconditional auto-approve and the runs transport's silent drop.
