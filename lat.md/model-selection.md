...
## Picker only offers configured providers

The chat picker filters `listModels()`'s full saved library down to models whose provider has a resolvable key or an authenticated OAuth session, so a pick can't fail at send time with no indication why.

`listModels()` (`src/main/models.ts`) returns every saved model regardless of whether a key is configured — it's also the source for the Providers-tab model editor, which needs the unfiltered set. Before that list reaches the picker, [[src/renderer/src/screens/Chat/hooks/useModelConfig.ts#useModelConfig]] filters it with `isProviderConfigured`, which resolves each model's env var via [[src/renderer/src/constants.ts#envKeyForSavedModel]] — a forward lookup built from the same `PROVIDERS.setup` / `NATIVE_ENV_KEY_ROUTES` / `OPENAI_COMPATIBLE_BASE_URLS` tables `providerRouteForEnvKey` already inverts, plus `expectedEnvKeyForUrl`/`customProviderEnvKey` for `custom`-routed and named-custom providers — and checks it against `getEnv()`. OAuth-only providers (`OAUTH_ONLY_PROVIDER_IDS`, mirroring `OAUTH_PROVIDERS`) are checked against `getOAuthProviderStatuses()` instead. A provider id neither table can resolve is kept visible rather than risk hiding a legitimately usable model on a mapping gap.

This is deliberately narrower than the Settings "LLM Providers" list is wide: Settings shows only providers with a key set, while the picker (pre-filter) showed the entire default-seeded library across every brand — the mismatch users would see between the two screens before this filter existed.
