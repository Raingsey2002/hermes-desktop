...
/**
 * Forward lookup: given a saved model's raw `provider` (the routing value
 * `setModelConfig` writes — "anthropic", "custom", "ollama-cloud", …), find
 * the env var whose value would need to be set for that model to actually be
 * usable. Built once from the same tables `providerRouteForEnvKey` already
 * inverts, so the two can't drift.
 *
 * Returns `null` when no table maps this provider to a known key — callers
 * should treat that as "can't determine" rather than "unconfigured".
 */
const PROVIDER_ENV_KEY_INDEX: ReadonlyMap<string, string> = (() => {
  ...
  for (const p of PROVIDERS.setup as ReadonlyArray<SetupRoute>) {
    if (p.envKey) index.set(p.configProvider ?? p.id, p.envKey);
  }
  for (const [envKey, route] of Object.entries(NATIVE_ENV_KEY_ROUTES)) {
    if (route.provider !== "custom") index.set(route.provider, envKey);
  }
  return index;
})();

export function envKeyForSavedModel(
  provider: string,
  baseUrl: string,
  providerLabel?: string,
): string | null {
  if (provider === "custom") {
    if (providerLabel) return customProviderEnvKey(providerLabel);
    return baseUrl ? expectedEnvKeyForUrl(baseUrl) : null;
  }
  const known = PROVIDER_ENV_KEY_INDEX.get(provider);
  if (known) return known;
  // Some brands (e.g. AtlasCloud) are seeded with a bare provider id and no
  // stored baseUrl instead of routing through `custom`. Fall back to the
  // compat table's canonical URL for that id so its key still resolves.
  const canonicalUrl = OPENAI_COMPATIBLE_BASE_URLS[provider];
  return canonicalUrl ? expectedEnvKeyForUrl(canonicalUrl) : null;
}

/** Provider ids that authenticate via OAuth rather than (only) an API key —
 *  `envKeyForSavedModel` can't see `auth.json`, so callers must additionally
 *  check `getOAuthProviderStatuses` for these before deciding "unconfigured". */
export const OAUTH_ONLY_PROVIDER_IDS: ReadonlySet<string> = new Set(
