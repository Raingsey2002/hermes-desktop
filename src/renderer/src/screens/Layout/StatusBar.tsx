 ...
 * live connection/gateway state and skill count that were previously buried.
 * Every field is real (sourced from `listProfiles` + `getConnectionConfig`);
 * nothing is fabricated, so if a value is unknown the chip is simply omitted
 * rather than shown with a placeholder. There is no longer a model chip here:
 * the config.yaml default it showed isn't necessarily the model in use for
 * any given conversation once a chat-input session override is picked, so a
 * single "current model" readout here would be misleading.
 */
