...
function Skills({
  ...
  // Free-form "install from URL or hub identifier" field — the CLI already
  // accepts either (`hermes skills install --help`), the UI just never
  // exposed a place to type one that isn't in the bundled catalog.
  const [addValue, setAddValue] = useState("");
  const [addBusy, setAddBusy] = useState(false);
  // Local-file install only makes sense in local mode: "a file on this
  // machine" doesn't mean anything once the agent is remote/SSH.
  const [isLocalMode, setIsLocalMode] = useState(false);

  useEffect(() => {
    let cancelled = false;
    void window.hermesAPI.isRemoteMode().then((remote) => {
      if (!cancelled) setIsLocalMode(!remote);
    });
    return () => {
      cancelled = true;
    };
  ...
  async function handleInstallFromField(): Promise<void> {
    const value = addValue.trim();
    if (!value || addBusy) return;
    setAddBusy(true);
    setError("");
    const result = await window.hermesAPI.installSkill(value, profile);
    setAddBusy(false);
    if (result.success) {
      setAddValue("");
      await loadInstalled();
      toast.success(t("skills.installSuccess", { name: value }));
    } else {
  ...
  async function handleInstallFromLocalFile(): Promise<void> {
    if (addBusy) return;
    const picked = await window.hermesAPI.selectSkillFile();
    if (!picked) return;
    setAddBusy(true);
    setError("");
    const result = await window.hermesAPI.installSkillFromPath(picked, profile);
    setAddBusy(false);
    if (result.success) {
      await loadInstalled();
      toast.success(t("skills.installFromFileSuccess"));
    } else {
      const msg = result.error || t("skills.installFailed");
      setError(msg);
      toast.error(msg);
  ...
  return (
    <div className="skills-container">
      ...
      {/* Add a skill not in the bundled catalog: a URL or hub identifier
          (the CLI already accepts either — this just exposes it), or, in
          local mode only, a SKILL.md/folder picked from disk. */}
      <div className="skills-add-row">
        <div className="skills-add-field">
          <Plus size={15} />
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") void handleInstallFromField();
            }}
            disabled={addBusy}
          />
        </div>
        <button
          className="btn btn-secondary btn-sm"
          onClick={() => void handleInstallFromField()}
        ...
        {isLocalMode && (
          <button
            className="btn btn-secondary btn-sm"
            onClick={() => void handleInstallFromLocalFile()}
            disabled={addBusy}
          >
            <FolderInput size={14} />
            {t("skills.addSkillFromFile")}
          </button>
        )}
      </div>
