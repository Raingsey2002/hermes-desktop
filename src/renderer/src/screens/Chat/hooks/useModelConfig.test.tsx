  ...
  beforeEach(() => {
    ...
    Object.defineProperty(window, "hermesAPI", {
      ...
      value: {
        ...
        // These tests exercise reload/grouping, not the "is this provider
        // configured" picker filter — a Proxy that reports every env var as
        // set keeps every model visible regardless of which key each
        // provider under test happens to resolve to.
        getEnv: vi.fn(
          async () => new Proxy({}, { get: () => "test-key" }) as never,
        ),
        getOAuthProviderStatuses: vi.fn(async () => ({})),
      },
    });
  });
