...
function Chat({
  ...
  // Fill the composer with `/<skill-name> ` — the existing slash-command
  // pipeline (slashExec.ts's `case "skill"`) already resolves and runs it
  // exactly as if the user had typed it; the picker just makes what's
  // installed discoverable instead of requiring it be typed from memory.
  const handleSelectSkill = useCallback((name: string) => {
    chatInputRef.current?.setText(`/${name} `);
  }, []);
