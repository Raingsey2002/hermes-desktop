...
// Category directory local (drag-in / file-picker) installs land under —
// distinct from any hub category, so an uploaded skill never collides with
// or gets mistaken for one the registry manages.
const LOCAL_SKILL_CATEGORY = "custom";

/** Directory-name-safe slug: lowercase, spaces/punctuation → `-`, trimmed. */
...
/**
 * Install a skill from a SKILL.md the user picked on disk (a direct file, or
 * a folder containing one — the CLI's `hermes skills install` only accepts a
 * hub identifier or an HTTP(S) URL, not a local path, so this bypasses it
 * entirely and copies straight into the profile's skills directory).
 *
 * `sourcePath` is whatever the native file/folder picker returned; not
 * validated against the skills-directory allowlist the way `getSkillContent`
 * is, since it's expected to be an arbitrary user-chosen location outside
 * it — that's the point of an upload flow.
 */
export function installSkillFromPath(
  ...
  try {
    ...
    if (!existsSync(resolved)) {
      return { success: false, error: "Selected path does not exist." };
    }
    const stat = statSync(resolved);
    // A picked folder is copied whole (the user explicitly scoped what to
    // include — reference assets alongside SKILL.md come along). A picked
    // *file* copies only that file: it must be named exactly SKILL.md (a
    // differently-named markdown file has no defined skill identity), and
    // copying its whole containing folder would risk pulling in unrelated
    // sibling files from wherever the user happened to pick it (Downloads,
    // Desktop, …).
    let skillFile: string;
    let sourceDirForFallbackName: string;
    let copyWholeFolder: string | null;
    if (stat.isDirectory()) {
      ...
      sourceDirForFallbackName = resolved;
      copyWholeFolder = resolved;
    } else {
      if (resolved.split(/[\\/]/).pop()?.toLowerCase() !== "skill.md") {
        return {
          success: false,
          error: "Select a SKILL.md file or its containing folder.",
        };
      }
      skillFile = resolved;
      sourceDirForFallbackName = resolve(resolved, "..");
      copyWholeFolder = null;
    }

    const content = readFileSync(skillFile, "utf-8").slice(0, 4000);
    const meta = parseSkillFrontmatter(content);
    const fallbackName =
      sourceDirForFallbackName.split(/[\\/]/).pop() || "skill";
    ...
    const destDir = join(
      profileHome(profile),
      "skills",
      LOCAL_SKILL_CATEGORY,
      slug,
    );
    if (existsSync(destDir)) {
      return {
        success: false,
        error: `A skill named "${slug}" is already installed under ${LOCAL_SKILL_CATEGORY}. Remove it first to reinstall.`,
      };
    }
    mkdirSync(destDir, { recursive: true });
    if (copyWholeFolder) {
      cpSync(copyWholeFolder, destDir, { recursive: true });
    } else {
