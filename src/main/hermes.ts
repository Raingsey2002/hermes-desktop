...
/**
 * Auth header for the active profile's gateway API, whichever mode it's
 * running in — local (`API_SERVER_KEY`), remote, or SSH (the remote/SSH
 * value from {@link getRemoteAuthHeader}). Exported so any gateway-API
 * client (not just the chat transport) can compose the right header without
 * re-deriving this local/remote split itself.
 */
