...
export function registerIpcHandlers(context: IpcContext): void {
  ...
  // Skills. Remote (HTTP) mode always routed to the dashboard's
  // /api/skills* — falling through to the local CLI there showed (and
  // mutated) the LOCAL machine's skills while connected to a remote (#578's
  // report). Local mode now prefers the SAME gateway API for the two
  // read-only operations (list/content), so skill data comes from one code
  // path regardless of mode; it falls back to the direct CLI only when the
  // local gateway can't be reached. Bundled skills stay local in every mode:
  // that list is the shipped catalog, not per-machine state. SSH keeps its
  // own tunnel-exec path (full local access already, via the tunnel).
  //
  // Install/uninstall stay on the local CLI path in local mode rather than
  // also routing through the gateway's hub endpoints: those endpoints SPAWN
  // the CLI server-side and return immediately ({ok, pid}) — success means
  // "started", not "completed" — whereas the local CLI path is synchronous
  // and gives a real pass/fail classification (classifySkillCliOutput).
  // Local mode has the better option available and keeps it; remote mode
  // has no local CLI to fall back to, so it accepts the async result.
  ipcMain.handle("list-installed-skills", async (_event, profile?: string) => {
    const conn = getConnectionConfig();
    if (conn.mode === "ssh" && conn.ssh)
      return sshListInstalledSkills(conn.ssh, profile);
    if (conn.mode === "remote")
      return gatewayListInstalledSkills(activeSshProfile(profile));
    const gatewayUp = await startGatewayWithRecovery(profile);
    if (gatewayUp) {
      try {
        return await gatewayListInstalledSkills(profile);
      } catch {
        // Gateway reported healthy but the API call itself failed —
        // fall through to the CLI below rather than surface an error.
      }
    }
  ...
  ipcMain.handle("get-skill-content", (_event, skillPath: string) => {
    ...
    // A gateway-marker path means this skill was LISTED via the gateway
    // (remote mode, or local mode's gateway-first list above) — route its
    // content lookup the same way regardless of the current mode, since
    // the path isn't a real local filesystem path in that case. The
    // marker always embeds the profile the skill was listed under, so no
    // fallback profile is needed here.
    if (skillPath.startsWith(GATEWAY_SKILL_PREFIX)) {
      return gatewayGetSkillContent(skillPath, activeSshProfile());
    }
  ...
  ipcMain.handle(
    ...
    (_event, identifier: string, _profile?: string) => {
      ...
      if (conn.mode === "remote")
        return gatewayInstallSkill(identifier, activeSshProfile(_profile));
  ...
  ipcMain.handle(
    ...
    (_event, name: string, _profile?: string) => {
      ...
      if (conn.mode === "remote")
        return gatewayUninstallSkill(name, activeSshProfile(_profile));
      return uninstallSkill(name, _profile);
    },
  );
  // Local-only: pick a SKILL.md (or its folder) from disk and copy it into
  // the profile's skills directory. There is no remote-filesystem
  // equivalent — a "local file" only means something on THIS machine — so
  // the renderer only offers this action in local mode.
  ipcMain.handle("select-skill-file", async (event) => {
    const win = BrowserWindow.fromWebContents(event.sender);
  ...
  ipcMain.handle(
    ...
    (_event, sourcePath: string, profile?: string) => {
      // Guard server-side too, not just by hiding the button: `profileHome`
      // resolves on THIS machine, so honoring this call in remote/SSH mode
      // would silently write into the wrong (local) profile directory.
      if (isRemoteMode()) {
        return {
          success: false,
          error:
            "Installing from a local file is only available in local mode.",
        };
      }
      return installSkillFromPath(sourcePath, profile);
    },
  );
